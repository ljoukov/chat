<script lang="ts">
	...
	const breadcrumbs = $derived.by((): Crumb[] => {
		...
		const matchedNav = [...demoNavigation]
...
<svelte:head>
	...
	<meta
		name="description"
		content="Gallery routes for @ljoukov/chat covering the live thread surface and isolated component renders."
	/>
</svelte:head>
		...
		<aside class="gallery-sidebar">
			...
			<div class="gallery-sidebar__content">
				<nav class="gallery-nav" aria-label="Gallery navigation">
					{#each demoNavigation as item (item.href)}
						<a class={`gallery-nav__link ${isActive(item.href) ? 'is-active' : ''}`} href={item.href}>
